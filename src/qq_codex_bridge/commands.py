...
def help_text(admin: bool = True, admin_private: bool = True) -> str:
    public = (
        "可用命令：\n"
        "/list - 命令清单与当前状态\n"
        "/new - 开启新 thread\n"
    ...
    if not admin:
        return public + "普通文本将发给当前 project 的 Codex 沙箱。"
    advanced = (
        "/project - 列出项目；/project <名> 切换；/project create <名> 创建\n"
    ...
    private = (
        ...
        "/yes /no - 审批应答\n"
        if admin_private
        else "群聊中的管理员仍固定使用 project 沙箱。\n"
    )
    return public + advanced + private + "普通文本将发给 Codex。"
